...
# ---------------------------------------------------------------------
# Legacy: spherical JSONL support (old pipeline)
# ---------------------------------------------------------------------
def frame_to_pointcloud_from_json_spherical(
    frame_obj: dict, pc_size: int, shift_arr: np.ndarray
) -> np.ndarray:
    """(Legacy) Convert one JSONL 'frame' object with spherical coords into (pc_size, 6).

    Uses the 'spherical' fields:
      range_m, azimuth_deg, elevation_deg, doppler_m_s, snr

    Output per point: [x, y, z, velocity, snr, range]
    """
...
# ---------------------------------------------------------------------
# New: Cartesian JSON support (your current .json file structure)
# ---------------------------------------------------------------------
def frame_to_pointcloud_from_json_cartesian(
    frame_data: dict, pc_size: int, shift_arr: np.ndarray
) -> np.ndarray:
    """Convert one JSON 'frameData' object with Cartesian pointCloud into (pc_size, 6).
    ...
    We build per-point features:
        [x, y, z, velocity, energy, range]
    where range = sqrt(x^2 + y^2 + z^2).

    All transforms (range cut, static clutter, flips, permutation, TOPK)
    are applied exactly like in the legacy pipeline.
    """
    ...
    for p in point_cloud:
        # Expect at least [x, y, z, vel, energy]
        if len(p) < 5:
            continue
    ...
    # Optional TOP-K selection by energy (column 4)
    if TOPK and data.shape[0] > TOPK_K:
        idx = np.argpartition(data[:, 4], -TOPK_K)[-TOPK_K:]
        data = data[idx]
...
def process_jsonl_file(jsonl_path: Path, pc_size: int, shift_arr: np.ndarray) -> np.ndarray:
    """Read one legacy .jsonl file and convert it to (n_frames, pc_size, 6)."""
    frames = []
    with open(jsonl_path, "r") as f:
        for line in f:
            ...
            pc_frame = frame_to_pointcloud_from_json_spherical(frame_obj, pc_size, shift_arr)
...
def process_json_file(json_path: Path, pc_size: int, shift_arr: np.ndarray) -> np.ndarray:
    """Read one new-format .json file and convert it to (n_frames, pc_size, 6)."""
    ...
    for entry in data_entries:
        frame_data = entry.get("frameData", None)
        if frame_data is None:
            continue
        pc_frame = frame_to_pointcloud_from_json_cartesian(frame_data, pc_size, shift_arr)
        frames.append(pc_frame)

    if not frames:
        raise ValueError(f"No frames parsed from {json_path}")
    return np.stack(frames, axis=0)  # (n_frames, pc_size, 6)
...
def main():
    ...
    input_dir = os.path.join(base_dir, getattr(cfg, "INPUT_DIR", "data/input_json"))
    output_dir = os.path.join(base_dir, getattr(cfg, "OUTPUT_DIR", "data/output_cartesian"))
    ...
    # Collect all .json and .jsonl files
    json_files = sorted(
        [
            Path(f)
            for f in glob.glob(os.path.join(input_dir, "**", "*.json"), recursive=True)
        ]
    )
    jsonl_files = sorted(
        [
    ...
    if not json_files and not jsonl_files:
        print(f"ERROR: No .json or .jsonl files found in {input_dir}")
        return
    ...
    if json_files:
        for fp in tqdm(json_files, desc="Processing .json", unit="file"):
            arr = process_json_file(fp, pc_size, shift_arr)
            dataset.append(arr)

    if jsonl_files:
        for fp in tqdm(jsonl_files, desc="Processing .jsonl (legacy)", unit="file"):
    ...
    if split_method == "sequential":
        ...
        train, test = split_random(
            dataset, test_ratio, seed=getattr(cfg, "SPLIT_SEED", 0)
        )
